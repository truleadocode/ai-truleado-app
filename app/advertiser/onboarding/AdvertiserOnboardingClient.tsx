...
type Phase = 'loading' | 'chat' | 'auth' | 'done'
...
export default function AdvertiserOnboardingClient({ user, advertiser }: Props) {
  ...
  async function initSession(forceNew = false) {
    ...
    if (!res.ok) {
      setPhase('chat')
      addSarahMessage("Hey! I'm Sarah from Truleado. Let's get you set up. What's your name?")
      return
    }
  ...
  async function startFresh() {
    ...
    setStep('greeting'); setResumeStep('greeting'); setIsResumePrompt(false)
    setPhase('loading')
  ...
  async function signInWithGoogle() {
    ...
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/auth/advertiser-callback?sk=${sk}` },
    })
  }
  ...
  async function sendMessage(text: string) {
    ...
    if (isResumePrompt) {
      ...
      addUserMessage(text); setInput('')
      if (isYes) { await continueFromStep(resumeStep); return }
      if (isNo) { addSarahMessage('No problem, starting fresh!'); await startFresh(); return }
      addSarahMessage('Just say "yes" to continue or "no" to start over.')
      return
    }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--surface)', display:'flex', flexDirection:'column', alignItems:'center', fontFamily:'Inter, sans-serif' }}>
      <div style={{ width:'100%', padding:'16px 24px', display:'flex', alignItems:'center', gap:10, borderBottom:'1px solid var(--border)', background:'var(--white)' }}>
        <div style={{ width:36, height:36, borderRadius:'50%', background:'var(--gold-bg)', border:'2px solid var(--gold-border)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16 }}>✨</div>
        <div>
        ...
        <div style={{ marginLeft:'auto', fontSize:12, color:'var(--text-3)' }}>For brands & agencies</div>
      </div>
        ...
        {(phase === 'chat' || phase === 'auth') && (
          <>
            <div style={{ flex:1, padding:'24px 16px', display:'flex', flexDirection:'column', gap:16, overflowY:'auto' }}>
              ...
              {phase === 'auth' && (
                <div style={{ padding:20, background:'var(--white)', border:'1px solid var(--border)', borderRadius:16, textAlign:'center' }}>
                  <p style={{ fontSize:13, color:'var(--text-2)', marginBottom:16 }}>Your info is saved — sign in to access your dashboard and start finding creators.</p>
                  <button onClick={signInWithGoogle} style={{ display:'inline-flex', alignItems:'center', gap:10, padding:'12px 24px', borderRadius:10, border:'1px solid var(--border)', background:'var(--white)', color:'var(--text)', fontSize:14, fontWeight:600, cursor:'pointer', fontFamily:'inherit', boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
            ...
            {phase === 'chat' && (
              <div style={{ padding:'12px 16px', borderTop:'1px solid var(--border)', background:'var(--white)', display:'flex', gap:8 }}>
                ...
                <button onClick={() => sendMessage(input)} disabled={!input.trim() || isThinking}
                  ...
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M14 8L2 2l2.5 6L2 14l12-6z" fill={input.trim() && !isThinking ? '#fff' : 'var(--text-3)'} /></svg>
                </button>
              </div>
            )}
          </>
