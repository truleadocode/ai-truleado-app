...
const STATUS_TABS = [
  { key: 'new',      label: 'New',      statuses: ['offered'] },
  { key: 'accepted', label: 'Accepted', statuses: ['confirmed','in_progress','complete'] },
  { key: 'passed',   label: 'Passed',   statuses: ['passed','rejected'] },
]

function statusBadge(status: string) {
  switch (status) {
    case 'offered':     return { label: 'New',            className: 'bg-gold-bg text-gold border border-gold-border' }
    case 'confirmed':
    case 'in_progress': return { label: 'Accepted',        className: 'bg-green-bg text-green border border-green-border' }
    ...
    case 'rejected':    return { label: 'Passed',          className: 'bg-muted text-muted-foreground border border-border' }
      ...
      {(!gigs || gigs.length === 0) && (
        <div className="bg-card border border-border rounded-2xl px-6 py-12 text-center shadow-sm">
          ...
          <p className="text-[13px] text-muted-foreground">
            {activeTab === 'new' ? 'New gigs will show up here once a brief matches your profile.' :
             activeTab === 'accepted' ? 'Gigs you accept will appear here.' :
