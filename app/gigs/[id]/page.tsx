...
function statusInfo(status: string) {
  switch (status) {
    case 'offered':     return { label: 'Offer received',   color:'var(--acc)',   bg:'var(--acc2)',          border:'var(--acc3)' }
...
export default function GigDetailPage() {
  ...
  if (!gig) {
    return (
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100vh' }}>
        <div style={{ width:32, height:32, border:'3px solid var(--line)', borderTopColor:'var(--acc)', borderRadius:'50%', animation:'spin 0.7s linear infinite' }} />
  ...
  return (
    <div style={{ padding:'24px 28px 40px', maxWidth:700 }}>
      {/* Back */}
      <Link href="/dashboard/gigs" style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize:12, fontWeight:600, color:'var(--muted)', textDecoration:'none', marginBottom:20 }}>
      ...
      {/* Header card */}
      <div style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:12, padding:'20px', marginBottom:16 }}>
        <div style={{ display:'flex', alignItems:'flex-start', gap:12, marginBottom:16 }}>
          <div style={{ width:48, height:48, borderRadius:12, background:'var(--bg3)', border:'1px solid var(--line)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22, flexShrink:0 }}>
            {gig.brand_revealed ? '🌿' : '🔒'}
          </div>
          <div style={{ flex:1 }}>
            ...
            <p style={{ fontSize:13, color:'var(--muted)' }}>{gig.platform} · {gig.deliverables_summary}</p>
            {!gig.brand_revealed && <p style={{ fontSize:11, color:'var(--muted)', marginTop:4 }}>Brand name revealed once you confirm the deal.</p>}
          </div>
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          {gig.budget_eur && (
            <div style={{ background:'var(--bg3)', border:'1px solid var(--line)', borderRadius:8, padding:'8px 14px', textAlign:'center' }}>
              <p style={{ fontSize:10, color:'var(--muted)', marginBottom:2 }}>Budget</p>
              <p style={{ fontSize:15, fontWeight:800, color:'var(--acc)' }}>{formatEur(gig.budget_eur)}</p>
            </div>
          )}
          {gig.respond_by && gig.status === 'offered' && (
            <div style={{ background:'var(--bg3)', border:'1px solid var(--line)', borderRadius:8, padding:'8px 14px', textAlign:'center' }}>
              <p style={{ fontSize:10, color:'var(--muted)', marginBottom:2 }}>Respond by</p>
          ...
          {gig.content_due_at && (
            <div style={{ background:'var(--bg3)', border:'1px solid var(--line)', borderRadius:8, padding:'8px 14px', textAlign:'center' }}>
              <p style={{ fontSize:10, color:'var(--muted)', marginBottom:2 }}>Content due</p>
              <p style={{ fontSize:13, fontWeight:700 }}>{new Date(gig.content_due_at).toLocaleDateString('en-GB', { month:'short', day:'numeric' })}</p>
            </div>
          )}
          {gig.go_live_at && (
            <div style={{ background:'var(--bg3)', border:'1px solid var(--line)', borderRadius:8, padding:'8px 14px', textAlign:'center' }}>
              <p style={{ fontSize:10, color:'var(--muted)', marginBottom:2 }}>Goes live</p>
        ...
        {gig.status === 'offered' && (
          <div style={{ display:'flex', gap:8, marginTop:16, paddingTop:16, borderTop:'1px solid var(--line)' }}>
            <button onClick={expressInterest} disabled={updatingStatus} style={{ flex:1, background:'var(--acc)', color:'#090E1A', border:'none', borderRadius:9, padding:'11px', fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
              I&apos;m interested
            </button>
      ...
      {/* Tabs */}
      <div style={{ display:'flex', gap:1, borderBottom:'1px solid var(--line)', marginBottom:16 }}>
        {['brief','chat'].map(t => (
          <button key={t} onClick={() => setTab(t as any)} style={{
            padding:'8px 18px', background:'transparent', border:'none', borderBottom: tab === t ? '2px solid var(--acc)' : '2px solid transparent',
            color: tab === t ? 'var(--fg)' : 'var(--muted)', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'inherit',
        ...
        <div>
          {gig.offer_notes && (
            <div style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:12, padding:'16px 18px', marginBottom:12 }}>
              <p style={{ fontSize:11, fontWeight:700, color:'var(--muted)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:10 }}>Brief from Sarah</p>
              <p style={{ fontSize:13, lineHeight:1.65, color:'var(--muted)' }}>{gig.offer_notes}</p>
            </div>
          )}
          ...
          {checklist.length > 0 && (
            <div style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:12, padding:'16px 18px' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
                <p style={{ fontSize:11, fontWeight:700, color:'var(--muted)', letterSpacing:'0.08em', textTransform:'uppercase' }}>Deliverables</p>
                <span style={{ fontSize:12, color:'var(--muted)' }}>{doneCount}/{checklist.length} done</span>
              </div>
              <div style={{ height:4, background:'var(--line)', borderRadius:4, marginBottom:14, overflow:'hidden' }}>
                <div style={{ height:'100%', background:'var(--acc)', borderRadius:4, width:`${(doneCount/checklist.length)*100}%`, transition:'width 0.3s' }} />
              </div>
              {checklist.map((item: any, i: number) => (
                <div key={i} onClick={() => toggleDeliverable(i)} style={{ display:'flex', alignItems:'flex-start', gap:10, padding:'10px 0', borderBottom: i < checklist.length - 1 ? '1px solid var(--line)' : 'none', cursor:'pointer' }}>
                  <div style={{
                    width:18, height:18, borderRadius:5, border: item.done ? 'none' : '1.5px solid var(--line)',
                  ...
                  <div>
                    <p style={{ fontSize:13, fontWeight:500, textDecoration: item.done ? 'line-through' : 'none', color: item.done ? 'var(--muted)' : 'var(--fg)' }}>{item.label || item.title || item.text || JSON.stringify(item)}</p>
                    {item.description && <p style={{ fontSize:12, color:'var(--muted)', marginTop:2 }}>{item.description}</p>}
                  </div>
                </div>
              ))}
            </div>
          ...
          {!gig.offer_notes && checklist.length === 0 && (
            <div style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:12, padding:'32px 24px', textAlign:'center' }}>
              <p style={{ fontSize:14, color:'var(--muted)' }}>No brief details yet. Sarah will add more info once confirmed.</p>
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'chat' && (
        <div>
          <div style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:12, overflow:'hidden' }}>
            <div style={{ maxHeight:420, overflowY:'auto', padding:'16px', display:'flex', flexDirection:'column', gap:8 }}>
              {messages.length === 0 && (
                <p style={{ textAlign:'center', fontSize:13, color:'var(--muted)', padding:'24px 0' }}>No messages yet</p>
              )}
              {messages.map((msg, i) => {
                  ...
                  <div key={msg.id}>
                    {showDate && (
                      <div style={{ textAlign:'center', margin:'8px 0' }}>
                        <span style={{ fontSize:11, color:'var(--muted)', background:'var(--bg3)', padding:'3px 10px', borderRadius:20 }}>{formatDay(msg.created_at)}</span>
                      </div>
                    )}
                    <div style={{ display:'flex', flexDirection: isInfluencer ? 'row-reverse' : 'row', gap:8, alignItems:'flex-end' }}>
                      {!isInfluencer && (
                        <div style={{ width:26, height:26, borderRadius:'50%', background:'var(--acc)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, fontWeight:700, color:'#090E1A', flexShrink:0 }}>SC</div>
                      )}
                      <div style={{ maxWidth:'70%' }}>
                        <div style={{
                          padding:'9px 13px', borderRadius: isInfluencer ? '12px 12px 4px 12px' : '12px 12px 12px 4px',
                          background: isInfluencer ? 'var(--acc)' : 'var(--bg3)',
                          color: isInfluencer ? '#090E1A' : 'var(--fg)',
                          fontSize:13, lineHeight:1.5,
                          border: isInfluencer ? 'none' : '1px solid var(--line)',
                        }}>{msg.content}</div>
                        <p style={{ fontSize:11, color:'var(--muted)', marginTop:2, textAlign: isInfluencer ? 'right' : 'left' }}>{formatTime(msg.created_at)}</p>
                      </div>
                    </div>
                  </div>
                )
            ...
            {/* Input */}
            <div style={{ padding:'12px', borderTop:'1px solid var(--line)', display:'flex', gap:8 }}>
              <input
                value={draft}
                onChange={e => setDraft(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage() } }}
                placeholder="Message Sarah..."
                style={{ flex:1, background:'var(--bg3)', border:'1px solid var(--line)', borderRadius:8, padding:'9px 12px', fontSize:13, color:'var(--fg)', fontFamily:'inherit', outline:'none' }}
              />
              <button onClick={sendMessage} disabled={sending || !draft.trim()} style={{
                background:'var(--acc)', border:'none', borderRadius:8, width:38, height:38, flexShrink:0,
